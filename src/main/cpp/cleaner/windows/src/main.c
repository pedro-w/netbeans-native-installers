 ...
 *        - a local file/folder name, e.g. "C:\foo\bar", up to a maximum of
 * 32767 chars and thus not subject to the original Windows limitation of 260
 * chars for a path name.
 ...
 *    Therefore the delete operations happens in parallel rather than in
 * sequence. If 64 threads have been spawned then wait for a thread to exit
 * before spawning a new one. (therefore never more than 64 threads) For each
...
const DWORD INITIAL_DELAY =
    2000; // 2 seconds seems to be enough to finish java process

const WCHAR *LINE_SEPARATOR = L"\r\n";
const WCHAR *UNC_PREFIX =
    L"\\\\?\\"; // Prefix for extended-length path in Win32 API
const WCHAR *UNC_STD_PREFIX =
    L"\\\\"; // Prefix for UNC paths, for example: \\servername\share\foo\bar
...
WCHAR *toWCHAR(char *charBuffer, DWORD size) {
  ...
  for (i = 0; i < size / 2; i++) {
    ...
    buffer[i] = ((unsigned char)realStringPtr[2 * i]) +
                (((unsigned char)realStringPtr[2 * i + 1]) << 8);
  }
  ...
  if (ptr != NULL) {
    while (counter < (*number)) {
      ...
      if ((ptr2 = search(ptr, LINE_SEPARATOR)) != NULL) {
        ...
        (*list)[counter] =
            (WCHAR *)LocalAlloc(LPTR, sizeof(WCHAR *) * (length + 1));
...
void deleteFile(WCHAR *filePath) {
  ...
  DWORD prefixLength =
      (filePath == search(filePath, UNC_STD_PREFIX)) ? 0 : UNC_PREFIX_LENGTH;
  ...
  if (GetFileAttributesExW(file, GetFileExInfoStandard, &attrs)) {
    if (attrs.dwFileAttributes &
        FILE_ATTRIBUTE_READONLY) { // if read-only attrib is set
      if (SetFileAttributesW(file, FILE_ATTRIBUTE_NORMAL) ==
          0) { // remove read-only attrib
    ...
    if (canDelete) {
      if (attrs.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
        while ((!RemoveDirectoryW(file) ||
                GetFileAttributesExW(file, GetFileExInfoStandard, &attrs)) &&
               ((count++) < MAX_ATTEMPTS)) {
          Sleep(SLEEP_DELAY);
        }
      } else {
        while ((!DeleteFileW(file) ||
                GetFileAttributesExW(file, GetFileExInfoStandard, &attrs)) &&
               ((count++) < MAX_ATTEMPTS)) {
          Sleep(SLEEP_DELAY);
        }
...
void getFreeIndexForNextThread(HANDLE *list, DWORD max, DWORD *counter) {
  ...
  while (1) {
    ...
    if (list[*counter] == INVALID_HANDLE_VALUE) {
      break;
    } else if (GetExitCodeThread(list[*counter], &code) != 0 &&
      ...
      *counter = (*counter) + 1;
      if ((*counter) == max && maxReached == 1) {
        *counter =
            WaitForMultipleObjects(max, list, FALSE, INFINITE) - WAIT_OBJECT_0;
      }
    }
  }
}
...
void removeItselfUsingCmd() {
  ...
  if (GetModuleFileNameA(0, currentFile, MAX_PATH)) {
    ...
    hTempFile = CreateFileA(tempFile, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                            CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (hTempFile != INVALID_HANDLE_VALUE) {
      char *command =
          LocalAlloc(LPTR, sizeof(char) * (lstrlenA(tempFile) +
                                           lstrlenA(currentFile) + 6));
      DWORD bytesNumber = 0;
      STARTUPINFO si;
      ...
      for (i = 0; i < 4; i++) {
        WriteFile(hTempFile, strings[i], lstrlenA(strings[i]), &bytesNumber,
                  NULL);
      }
      ...
      CreateProcess(0, command, 0, 0, FALSE,
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hi, PSTR pszCmdLine,
                   int nCmdShow) {
  ...
  HANDLE *runningThreads =
      (HANDLE *)LocalAlloc(LPTR, sizeof(HANDLE) * MAXIMUM_THREADS);
    ...
    if (fileList != 0) {
      ...
      if (files != NULL) {
        ...
        for (i = 0; i < number; i++) {
          ...
          if (file != NULL) {
            if (lstrlenW(file) > 0) {
              getFreeIndexForNextThread(runningThreads, MAXIMUM_THREADS,
                                        &threadCounter);
              runningThreads[threadCounter] = CreateThread(
                  NULL, 0, &deleteFileThread, (LPVOID)file, 0, &dwThread);
              threadCounter++;
              if (threadCounter == MAXIMUM_THREADS)
                allThreadsUsed = 1;
            }
          }
        }
