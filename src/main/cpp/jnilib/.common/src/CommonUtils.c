...
jbyteArray getStringBytes(JNIEnv *jEnv, jstring jString) {
  ...
  if (jString != NULL) {
    jmethodID jGetBytesMethod = (*jEnv)->GetMethodID(
        jEnv, (*jEnv)->GetObjectClass(jEnv, jString), "getBytes", "()[B");

    if (jGetBytesMethod != NULL) {
      jbyteArray jBuffer =
          (jbyteArray)(*jEnv)->CallObjectMethod(jEnv, jString, jGetBytesMethod);
...
jstring newStringFromJByteArray(JNIEnv *jEnv, jbyteArray jByteArray,
                                int length) {
  jstring result = NULL;

  jclass jStringClass = (*jEnv)->FindClass(jEnv, "java/lang/String");
  if (jStringClass != NULL) {
    jmethodID jStringConstructor =
        (*jEnv)->GetMethodID(jEnv, jStringClass, "<init>", "([BII)V");

    if (jStringConstructor != NULL) {
      result = (jstring)(*jEnv)->NewObject(
          jEnv, jStringClass, jStringConstructor, jByteArray, 0, length);
...
jstring newStringFromJCharArray(JNIEnv *jEnv, jcharArray jCharArray,
                                int length) {
  jstring result = NULL;
  ...
  if (jStringClass != NULL) {
    jmethodID jStringConstructor =
        (*jEnv)->GetMethodID(jEnv, jStringClass, "<init>", "([CII)V");

    if (jStringConstructor != NULL) {
      result = (jstring)(*jEnv)->NewObject(
          jEnv, jStringClass, jStringConstructor, jCharArray, 0, length);
    }
    (*jEnv)->DeleteLocalRef(jEnv, jStringClass);
  }
...
jstring getStringWithLength(JNIEnv *jEnv, const char *chars, int length) {
  ...
  if (chars != NULL) {
    if (length == 0) {
      ...
      if (jByteArray != NULL) {
        (*jEnv)->SetByteArrayRegion(jEnv, jByteArray, 0, length,
                                    (jbyte *)chars);
...
jstring getStringWithLengthW(JNIEnv *jEnv, const wchar_t *chars, int length) {
  ...
  if (chars != NULL) {
    if (length == 0) {
      ...
      if (jCharArray != NULL) {
        (*jEnv)->SetCharArrayRegion(jEnv, jCharArray, 0, length,
                                    (jchar *)chars);
...
char *getStringFromMethod(JNIEnv *jEnv, jobject object,
  ...
  if (clazz != NULL) {
    jmethodID method =
        (*jEnv)->GetMethodID(jEnv, clazz, methodName, "()Ljava/lang/String;");
    if (method != NULL) {
...
wchar_t *getWideStringFromMethod(JNIEnv *jEnv, jobject object,
                                 const char *methodName) {
  wchar_t *result = NULL;
...
void throwException(JNIEnv *jEnv, const char *message) {
  jclass clazz = (*jEnv)->FindClass(
      jEnv, "org/netbeans/installer/utils/exceptions/NativeException");
  if (clazz != NULL) {
...
void writeLog(JNIEnv *jEnv, int level, const char *message) {
  ...
  jclass clazz =
      (*jEnv)->FindClass(jEnv, "org/netbeans/installer/utils/LogManager");
  if (clazz != NULL) {
    jmethodID method = (*jEnv)->GetStaticMethodID(jEnv, clazz, "log",
    ...
    if (method != NULL) {
      ...
      char *string =
          (char *)MALLOC(sizeof(char) * (prefix_length + message_length + 1));
      int i = 0;
      ...
      if (jMessage != NULL) {
        (*jEnv)->CallStaticVoidMethod(jEnv, clazz, method, (jint)level,
                                      jMessage);
...
int createDirs(JNIEnv *jEnv, jstring jPath) {
  ...
  if (jFileClass != NULL) {
    jmethodID jFileConstructor = (*jEnv)->GetMethodID(
        jEnv, jFileClass, "<init>", "(Ljava/lang/String;)V");
    jmethodID jGetParentFileMethod = (*jEnv)->GetMethodID(
        jEnv, jFileClass, "getParentFile", "()Ljava/io/File;");
    jmethodID jExistsMethod =
        (*jEnv)->GetMethodID(jEnv, jFileClass, "exists", "()Z");
    jmethodID jMkdirsMethod =
        (*jEnv)->GetMethodID(jEnv, jFileClass, "mkdirs", "()Z");

    if ((jFileConstructor != NULL) && (jGetParentFileMethod != NULL) &&
        (jExistsMethod != NULL) && (jMkdirsMethod != NULL)) {
      jobject jFile =
          (*jEnv)->NewObject(jEnv, jFileClass, jFileConstructor, jPath);
      if (jFile != NULL) {
        if (!((*jEnv)->CallBooleanMethod(jEnv, jFile, jExistsMethod))) {
          jobject jParent =
              (*jEnv)->CallObjectMethod(jEnv, jFile, jGetParentFileMethod);
          if (jParent != NULL) {
...
unsigned char *getByteFromMultiString(JNIEnv *jEnv, jobjectArray jObjectArray,
                                      unsigned long *size) {
