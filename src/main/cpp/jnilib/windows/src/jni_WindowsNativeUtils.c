...
Java_org_netbeans_installer_utils_system_WindowsNativeUtils_isCurrentUserAdmin0(
  ...
  while (TRUE) {
    ...
    if (!InitializeSecurityDescriptor(adminDescriptor,
    ...
    if (!AccessCheck(adminDescriptor, duplToken, ACCESS_READ, &mapping, &ps,
                     &structSize, &status, &result)) {
      throwException(jEnv, "Native error");
      break;
    }
...
Java_org_netbeans_installer_utils_system_WindowsNativeUtils_getFreeSpace0(
  ...
  if (GetDiskFreeSpaceExW(path, (PULARGE_INTEGER)&bytes, NULL, NULL)) {
    ...
    // (2^32 * high + low) = 2^32 + 2^32 + ... (bytes.High times total) +
    // bytes.Low can be relatively expensive on big sizes (peta bytes and more)
    while (h > 0) {
      h--;
      size += 4294967296L;
    }
...
Java_org_netbeans_installer_utils_system_WindowsNativeUtils_createShortcut0(
    JNIEnv *jEnv, jobject jObject, jobject jShortcut) {
  unsigned short *shortcutPath =
      getWideStringFromMethod(jEnv, jShortcut, "getPath");
  unsigned short *targetPath =
      getWideStringFromMethod(jEnv, jShortcut, "getTargetPath");
  unsigned short *description =
      getWideStringFromMethod(jEnv, jShortcut, "getDescription");
  unsigned short *iconPath =
      getWideStringFromMethod(jEnv, jShortcut, "getIconPath");
  jint iconIndex = getIntFromMethod(jEnv, jShortcut, "getIconIndex");
  unsigned short *workingDirectory =
      getWideStringFromMethod(jEnv, jShortcut, "getWorkingDirectoryPath");
  unsigned short *arguments =
      getWideStringFromMethod(jEnv, jShortcut, "getArgumentsString");
  ...
  if (SUCCEEDED(tempResult)) {
    ...
    tempResult = shell->lpVtbl->QueryInterface(shell, &IID_IPersistFile,
                                               (void **)&persistFile);
    ...
    if (SUCCEEDED(tempResult)) {
      ...
      if ((errorCode == 0) && (description != NULL)) {
        if (WCSLEN(description) < MAX_PATH) {
          ...
          unsigned short *desc =
              (unsigned short *)MALLOC(sizeof(unsigned short) * MAX_PATH);
      ...
      if ((errorCode == 0) && (workingDirectory != NULL)) {
        if (!SUCCEEDED(
                shell->lpVtbl->SetWorkingDirectory(shell, workingDirectory))) {
      ...
      if ((errorCode == 0) && (iconPath != NULL)) {
        if (!SUCCEEDED(
                shell->lpVtbl->SetIconLocation(shell, iconPath, iconIndex))) {
      ...
      if (errorCode == 0) {
        if (mkdirsW(jEnv, shortcutPath)) {
          if (!SUCCEEDED(
                  persistFile->lpVtbl->Save(persistFile, shortcutPath, TRUE))) {
...
Java_org_netbeans_installer_utils_system_WindowsNativeUtils_createProcessWithoutHandles0(
    JNIEnv *jEnv, jobject jObject, jstring jCommand) {
...
Java_org_netbeans_installer_utils_system_WindowsNativeUtils_checkAccessTokenAccessLevel0(
  ...
  if (pSD == NULL) {
    throwException(jEnv,
                   "Unable to allocate memory to store security descriptor.\n");
    return -1;
  }
  ...
  if (!OpenThreadToken(GetCurrentThread(), TOKEN_DUPLICATE | TOKEN_QUERY, FALSE,
                       &hToken)) {
  ...
  if (!AccessCheck(pSD, hToken, DesiredAccess, &GenericMapping, &PrivilegeSet,
                   &PrivSetSize, &GrantedAccess, &bAccessGranted)) {
