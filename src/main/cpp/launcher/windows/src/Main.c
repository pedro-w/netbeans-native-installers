...
void initMainWindow(LauncherProperties *props, HINSTANCE hInstance) {
  if (!isSilent(props)) {
    ...
    hwndMain = CreateWindow(
        mainClassName, mainTitle,
        // WS_OVERLAPPED | WS_EX_TOOLWINDOW,
        WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_BORDER | WS_DLGFRAME |
            WS_SYSMENU | WS_MINIMIZEBOX /* | WS_THICKFRAME | WS_MAXIMIZEBOX*/
        ,
...
void initErrorTitleWindow(LauncherProperties *props, HINSTANCE hInstance) {
  if (!isSilent(props)) {
    ...
    hwndErrorTitle = CreateWindowExW(
        0, WC_STATICW, WC_STATICW, WS_CHILD, rcClient.left + 10, 15,
        rcClient.right - 20, 20, hwndMain, NULL, hInstance, NULL);
    if (hwndErrorTitle) {
...
void initErrorDetailWindow(LauncherProperties *props, HINSTANCE hInstance) {
  if (!isSilent(props)) {
    RECT rcClient;
    GetClientRect(hwndMain, &rcClient);
    hwndErrorDetail = CreateWindowExW(
        0, WC_STATICW, WC_STATICW, WS_CHILD, rcClient.left + 10, 40,
        rcClient.right - 20, 80, hwndMain, NULL, hInstance, NULL);
    if (hwndErrorDetail) {
      LOGFONT lfDetail;
      HFONT detailFont;
      HFONT hFont = (HFONT)GetStockObject(DEFAULT_GUI_FONT);
...
void initProgressTitleWindow(LauncherProperties *props, HINSTANCE hInstance) {
  if (!isSilent(props)) {
    ...
    GetClientRect(hwndMain, &rcClient);
    hwndProgressTitle = CreateWindowExW(
        0, WC_STATICW, WC_STATICW, WS_CHILD | WS_VISIBLE, rcClient.left + 10,
        (rcClient.bottom - cyVScroll) / 2 - height, rcClient.right - 20, height,
    ...
    if (hwndProgressTitle) {
      ...
      SendMessage(hwndProgressTitle, WM_SETFONT, (WPARAM)progressTitleFont,
                  FALSE);
      // DeleteObject(detailFont);
...
void initProgressWindow(LauncherProperties *props, HINSTANCE hInstance) {
  if (!isSilent(props)) {
    ...
    cyVScroll = GetSystemMetrics(SM_CYVSCROLL);
    GetClientRect(hwndMain, &rcClient);
    hwndPB = CreateWindowExW(
        0, PROGRESS_CLASSW, NULL, WS_CHILD | WS_VISIBLE | PBS_SMOOTH,
        rcClient.left + 10, (rcClient.bottom - cyVScroll) / 2,
        rcClient.right - 20, cyVScroll, hwndMain, NULL, hInstance, NULL);
...
void initExitButton(LauncherProperties *props, HINSTANCE hInstance) {
  if (!isSilent(props)) {
    ...
    hwndButton = CreateWindowExW(
        ...
        rcClient.right - 20 - buttonWidth, rcClient.bottom - 10 - buttonHeight,
        buttonWidth, buttonHeight, hwndMain, (HMENU)BTN_EXIT, hInstance, 0);
    if (hwndButton) {
      LOGFONT lfButton;
      HFONT buttonFont;
      HFONT hFont = (HFONT)GetStockObject(DEFAULT_GUI_FONT);
...
void showError(LauncherProperties *props, LPCTSTR error,
  ...
  totalLength = getLength(errorMessage);
...
#else
    wvsprintfA(result, errorMessage, ap);
#endif
  }
  va_end(ap);

  if (!isSilent(props)) {
    HANDLE events[] = {buttonPressed, closingWindowsRequired};
...
BOOL InitInstance(LauncherProperties *props, HINSTANCE hInstance, int iCmdShow,
...
BOOL InitApplication(LauncherProperties *props, HINSTANCE hInstance) {
  if (isSilent(props)) {
    ...
    wndclass.hIconSm =
        (HICON)LoadImage(GetModuleHandle(NULL), MAKEINTRESOURCE(100),
...
DWORD isTerminated(LauncherProperties *props) {
  if (props->status == ERROR_USER_TERMINATED) {
    writeMessage(props, OUTPUT_LEVEL_DEBUG, 0, TEXT("... already terminated"),
                 1);
    return 1;
  }
  if (WAIT_OBJECT_0 == WaitForSingleObject(closingWindowsRequired, 0)) {
    writeMessage(props, OUTPUT_LEVEL_DEBUG, 0, TEXT("... terminate signal!"),
                 1);
...
void addProgressPosition(LauncherProperties *props, DWORD add) {
  ...
  if (add > 0) {
    ...
    do {
      multiply(mult1 =
                   newint64_t(totalProgressSize->Low, totalProgressSize->High),
               lastCheckedStep);
      multiply(mult2 =
                   newint64_t(totalProgressSize->Low, totalProgressSize->High),
               lastCheckedStep + 1);
      multiply(currMult = newint64_t(currentProgressSize->Low,
                                     currentProgressSize->High),
               steps);
...
void setProgressTitleString(LauncherProperties *props, LPCTSTR message) {
  if (isSilent(props))
    return;
  SetWindowText(hwndProgressTitle, message);
...
void setErrorTitleString(LauncherProperties *props, LPCTSTR message) {
  if (isSilent(props))
    return;
  SetWindowText(hwndErrorTitle, message);
...
void setErrorDetailString(LauncherProperties *props, LPCTSTR message) {
  if (isSilent(props))
    return;
  SetWindowText(hwndErrorDetail, message);
...
void setButtonString(LauncherProperties *props, LPCTSTR message) {
  if (isSilent(props))
    return;
  SetWindowText(hwndButton, message);
  UpdateWindow(hwndButton);
  UpdateWindow(hwndMain);
}
...
void showMessage(LauncherProperties *props, LPCTSTR message,
                 const DWORD varArgsNumber, ...) {
  DWORD totalLength = getLength(message);
  va_list ap;
  DWORD counter = 0;
  LPTSTR result = NULL;
  writeMessage(props, OUTPUT_LEVEL_DEBUG, 0, showMessageTitle, 0);
...
#else
  wvsprintfA(result, message, ap);
#endif
  va_end(ap);
  writeMessage(props, OUTPUT_LEVEL_DEBUG, 0, result, 1);
  if (!isSilent(props)) {
    hideLauncherWindows(props);
    MessageBox(hwndMain, result, getI18nProperty(props, MSG_MESSAGEBOX_TITLE),
               MB_OK);
  }
}
...
varArgsNumber, ...) { DWORD totalLength = getLengthA(message); va_list ap; DWORD
counter=0; char * result = NULL;
...
DWORD WINAPI launcherThread(void *ptr) {
  HANDLE events[] = {initializationSuccess, initializationFailed};
  DWORD result;
...
void createLauncherThread(LauncherProperties *props) {
  DWORD threadId;
  if (CreateThread(NULL, 0, &launcherThread, (LPVOID)props, 0, &threadId) ==
      NULL) {
    SetEvent(closingWindowsConfirmed);
  }
}

DWORD createGui(LauncherProperties *props, HINSTANCE hInstance, HINSTANCE hi,
                int nCmdShow) {
  (void)hi;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hi, LPSTR lpCmdLine,
                   int nCmdShow) {
  DWORD exitCode = 1;
...
int WINAPI MyMain(void) {
  ...
  exitCode = WinMain(GetModuleHandle(NULL), NULL, NULL,
